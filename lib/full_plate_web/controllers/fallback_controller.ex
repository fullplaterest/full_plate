defmodule FullPlateWeb.FallbackController do
   @moduledoc """
  ...
  def call(conn, {:error, %Ecto.Changeset{errors: _errors} = changeset}) do
    errors = FullPlateWeb.Jsons.ChangesetJson.translate_errors(changeset)
    Logger.error("Error fallback unprocessable_entity ecto changeset. Errors: #{inspect(errors)}")

    conn
    |> put_status(:unprocessable_entity)
    |> put_view(json: FullPlateWeb.Jsons.ChangesetJson)
  ...
  def call(conn, {:error, :bad_request}) do
    ...
    |> put_view(json: FullPlateWeb.Jsons.ErrorJson)
